...
REGISTER_PROBLEM(AOC_2019_Day7)
{
    auto in = string_get_integers(file_read(PRACTICE_ROOT "/advent_of_code/2019/inputs/day_7.txt"));
//    in = string_get_integers("3, 15, 3, 16, 1002, 16, 10, 16, 1, 16, 15, 15, 4, 15, 99, 0, 0");
    ...
    operations[OpCode::output] = [&](Instruction& inst) { output = read(inst.parameters[0]); };
    ...
    std::vector<int> backup = in;
    auto runProgram = [&](const std::vector<int>& inVals)
    {
        in = backup;
            ...
            for (int C = 0; C < 5; C++)
            {
                for (int D = 0; D < 5; D++)
                {
                    for (int E = 0; E < 5; E++)
                    {
                        ...
                        int res = runProgram({ A, 0 });
                        res = runProgram({ B, res });
                        res = runProgram({ C, res });
                        res = runProgram({ D, res });
                        res = runProgram({ E, res });
